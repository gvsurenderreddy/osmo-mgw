			       ...
			       const char *text, int32_t *tsdelta_out)
{
	...
	timestamp_error =
		ts_aligment_error(sstate, state->packet_duration,
						 timestamp);
				      ...
				      uint32_t timestamp)
{
	...
	timestamp_error = ts_aligment_error(
...
{
	LOGP(DLMGCP, LOGL_DEBUG, "endpoint:%x mgcp_rtp_processing_default()\n", ENDPOINT_NUMBER(endp));
	return 0;
}
...
{
	LOGP(DLMGCP, LOGL_DEBUG, "endpoint:%x mgcp_setup_rtp_processing_default()\n", ENDPOINT_NUMBER(endp));
	return 0;
}
...
{
	LOGP(DLMGCP, LOGL_DEBUG, "endpoint:%x conn:%s mgcp_get_net_downlink_format_default()\n", ENDPOINT_NUMBER(endp), mgcp_conn_dump(conn->conn));
...
#if 0
	DEBUGP(DLMGCP, "endpoint:%x payload hdr payload %u -> endp payload %u\n",
			...
			char *buf, int bufsize)
{
	...
	if (rc < 0) {
		LOGP(DLMGCP, LOGL_ERROR, "endpoint:%x failed to receive packet, errno: %d/%s\n",
		     ...
		     struct mgcp_conn_rtp *conn_dst)
{
	...
	LOGP(DLMGCP, LOGL_DEBUG, "endpoint:%x ",
	     ENDPOINT_NUMBER(endp));
	LOGPC(DLMGCP, LOGL_DEBUG, "receiveing from %s %s %d\n",
	      conn_src->conn->name, inet_ntoa(addr.sin_addr), ntohs(addr.sin_port));
	LOGP(DLMGCP, LOGL_DEBUG, "endpoint:%x conn:%s\n",
	...
	if (memcmp(&addr.sin_addr, &conn_src->end.addr, sizeof(addr.sin_addr)) != 0) {
		LOGP(DLMGCP, LOGL_ERROR,
	...
	if (conn_src->end.rtp_port != addr.sin_port &&
		...
		LOGPC(DLMGCP, LOGL_ERROR,
		      ...
		      ntohs(conn_src->end.rtp_port), ntohs(conn_src->end.rtcp_port));
		LOGP(DLMGCP, LOGL_ERROR,
		     "endpoint:%x packet tossed\n",
     		     ENDPOINT_NUMBER(endp));
		return -1;
			...
			struct mgcp_rtp_end *rtp_end, int endpno)
{
	...
	if (mgcp_create_bind(source_addr, &rtp_end->rtp,
		...
		LOGP(DLMGCP, LOGL_ERROR, "endpoint:%x failed to create RTP port: %s:%d\n",
		     endpno, source_addr, rtp_end->local_port);
		goto cleanup0;
	}

	if (mgcp_create_bind(source_addr, &rtp_end->rtcp,
			     rtp_end->local_port + 1) != 0) {
		LOGP(DLMGCP, LOGL_ERROR, "endpoint:%x failed to create RTCP port: %s:%d\n",
		     endpno, source_addr, rtp_end->local_port + 1);
		goto cleanup1;
	}
	...
	if (osmo_fd_register(&rtp_end->rtp) != 0) {
		LOGP(DLMGCP, LOGL_ERROR, "endpoint:%x failed to register RTP port %d\n",
		     endpno, rtp_end->local_port);
		goto cleanup2;
	}

	rtp_end->rtcp.when = BSC_FD_READ;
	if (osmo_fd_register(&rtp_end->rtcp) != 0) {
		LOGP(DLMGCP, LOGL_ERROR, "endpoint:%x failed to register RTCP port %d\n",
		     endpno, rtp_end->local_port + 1);
		goto cleanup3;
	}
			   ...
			   struct mgcp_conn_rtp *conn)
{
	...
	if (end->rtp.fd != -1 || end->rtcp.fd != -1) {
		LOGP(DLMGCP, LOGL_ERROR, "endpoint:%x %u was already bound on conn:%s\n",
		     ENDPOINT_NUMBER(endp), rtp_port, mgcp_conn_dump(conn->conn));
	...
	return bind_rtp(endp->cfg, mgcp_net_src_addr(endp), end, ENDPOINT_NUMBER(endp));
}
