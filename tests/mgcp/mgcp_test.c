...
static void test_messages(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(tests); i++) {
		...
		if (last_endpoint != -1) {
			...
			conn = mgcp_conn_get_rtp(&endp->conns, 1);
			if (conn) {
				OSMO_ASSERT(conn);
				...
				if ((conn->conn->mode & CONN_UNMODIFIED) == 0) {
					printf("Connection mode: %d:%s%s%s%s\n",
					       ...
					       ~MGCP_CONN_RECV_SEND ? " LOOP" :
					       "");
					fprintf(stderr,
						"RTP output %sabled, NET output %sabled\n",
						conn->
						end.output_enabled ? "en" :
						"dis",
						conn->
						end.output_enabled ? "en" :
						"dis");
				...
				//OSMO_ASSERT(conn->end.output_enabled == (conn->conn->mode & MGCP_CONN_RECV_ONLY ? 1 : 0));
...
static void test_packet_error_detection(int patch_ssrc, int patch_ts)
{
	...
	mgcp_conn_alloc(NULL, &endp.conns, 4711, MGCP_CONN_TYPE_RTP,
			"test-connection");
	conn = mgcp_conn_get_rtp(&endp.conns, 4711);
	OSMO_ASSERT(conn);
...
static void test_multilple_codec(void)
{
	...
	endp = &cfg->trunk.endpoints[last_endpoint];
	conn = mgcp_conn_get_rtp(&endp->conns, 90210);
	OSMO_ASSERT(conn);
	OSMO_ASSERT(conn->end.codec.payload_type == 18);
	OSMO_ASSERT(conn->end.alt_codec.payload_type == 97);
	...
	endp = &cfg->trunk.endpoints[last_endpoint];
	conn = mgcp_conn_get_rtp(&endp->conns, 815);
	OSMO_ASSERT(conn);
	...
	endp = &cfg->trunk.endpoints[last_endpoint];
	conn = mgcp_conn_get_rtp(&endp->conns, 32168);
	OSMO_ASSERT(conn);
	OSMO_ASSERT(conn->end.codec.payload_type == 18);
	OSMO_ASSERT(conn->end.alt_codec.payload_type == -1);
	...
	endp = &cfg->trunk.endpoints[last_endpoint];
	conn = mgcp_conn_get_rtp(&endp->conns, 3);
	OSMO_ASSERT(conn);
